...
on_install() {
  ...
  if [ -f /data/dnsmasq/dnsmasq.conf ]; then
    ui_print "[2/6] Found dnsmasq.conf, will not copy a default one"
  else
    ui_print "[2/6] Extracting dnsmasq.conf"
    cp "$MODPATH/custom/dnsmasq.conf" '/data/dnsmasq/dnsmasq.conf'
  fi
...
set_permissions() {
  ...
  ui_print "[4/6] Installing to /system/xbin..";
  ...
  chmod -R 664 /data/dnsmasq;
  chown -R 0:0 /data/dnsmasq;
  find /data/dnsmasq -type d -exec chmod 755 {} \+;
  find /data/dnsmasq -type f -exec chmod 664 {} \+;
  set_perm /data/dnsmasq/dnsmasq.conf 0 0 0600
